...
std::map<std::string, double> compute_daily_kcal(
  ...
  if (blines.size() >= 2) {
    for (size_t i = 1; i < blines.size(); ++i) {
      ...
      Unit u = (trim(c[5]) == "mL" || trim(c[5]) == "ml") ? Unit::ML : Unit::G;

      auto p = db.get_by_id(pid);
      if (!p.has_value()) continue;

      double kcal_total = qty * p->kcal_per_100 / 100.0;
      double kcal_day = kcal_total / (double)bdays;

      for (int k = 0; k < bdays; ++k) {
        Date d = add_days(bstart, k);
        if (!in_range(d, start, end)) continue;
        per_day[format_date(d)] += kcal_day;
      }
    }
  }
